...
func TestLaneSkipsJobWithExistingReceipt(t *testing.T) {
	...
	if d := lk.Apply(contract.KernelOp{OpID: "pre", Actor: "lane", Writes: []contract.ResourceWrite{
		{Ref: contract.ResourceRef{Kind: "receipt", ID: "job_k_ev-job"}, Kind: contract.OpCreate, Fields: map[string]any{"job_id": "job_k_ev-job", "effect_id": "job_k_ev-job", "outcome": "ok"}}}},
