...
// Full hub integration over native TLS: mnemond serves push/pull/status with the dev self-signed
...
func TestMnemondServesSyncOverTLS(t *testing.T) {
	work := t.TempDir()
	...
	defer func() {
		cancel()
		if err := <-done; err != nil {
			t.Fatalf("mnemond exited with error: %v", err)
		}
	}()
	...
	fields := map[string]any{"content": "pushed through mnemond"}
...
var listenLine = regexp.MustCompile(`mnemond: listening on (https?://[^\s]+) `)
...
func waitForListen(t *testing.T, out *lockedBuffer) string {
	...
	t.Fatalf("mnemond did not report a listen address:\n%s", out.String())
	return ""
}
