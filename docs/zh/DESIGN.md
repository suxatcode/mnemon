...
| **LLM-as-Subagent** | 被调度的子进程 | 被编排者（响应 MCP 工具调用） | MemCP |
...
`mnemon remember` 是写入记忆的核心命令。整个管线在一个 SQLite 事务中原子执行。
...
`mnemon recall --smart` 是 Mnemon 的核心检索算法。它结合意图检测、多信号锚点选择、Beam Search 图遍历和多因子重排序，实现了意图感知的图增强检索。
...
`mnemon diff` 在存储前检测重复和冲突，避免知识库膨胀。
...
1. 对所有 insight 进行 KeywordSearch，取 top-5
2. 对每个匹配项计算 ContentSimilarity（token 重叠率）
3. 根据相似度和否定模式判断建议动作：

| 相似度 | 否定模式 | 建议 |
|--------|---------|------|
| < 0.50 | — | **ADD**（新增） |
| ≥ 0.50 且检测到否定词 | 是 | **CONFLICT**（冲突） |
| > 0.90 | 否 | **DUPLICATE**（重复） |
| 0.50 ~ 0.90 | 否 | **UPDATE**（更新） |

**否定模式检测**：`not`, `no longer`, `don't`, `never`, `switched from`, `replaced`, `不`, `没有`, `放弃`, `改为`

### 8.2 典型工作流

LLM 在 remember 之前先调用 diff：

```bash
# Step 1: 检查是否重复
mnemon diff "选择 PostgreSQL 作为主数据库"
# → 返回: CONFLICT with "选择 SQLite 作为存储" (sim=0.65, negation detected)

# Step 2: LLM 决定更新而非新增
mnemon forget <old_id>
mnemon remember "选择 PostgreSQL 替代 SQLite 作为主数据库" --cat decision --imp 5
```
...
| recall --smart → 锚点 | 关键词 + 时间 | 关键词 + 向量 + 时间 |
| recall --smart → 遍历 | 纯结构分 | 结构 + 语义相似度 |
| recall --smart → 重排 | KW + Entity + Graph | KW + Entity + Similarity + Graph |
...
用户消息
    │
    ▼
  Hook ─── 自动召回 ──→ [Past memory] 注入到 LLM 上下文
    │
    ▼
  CLAUDE.md ── "使用过去的记忆；评估是否需要记住"
    │
    ▼
  Skill ── "命令语法：mnemon diff → mnemon remember --cat ..."
```

**Layer 1: Hook（自动召回）**

安装到 `~/.claude/hooks/`，在每次用户发送消息时自动执行：
...
结果以 `[Past memory]` 标记注入到 LLM 上下文中。这确保了：
- 无需 LLM 主动发起，每次对话都有相关记忆
- 零延迟感知——记忆在 LLM 思考前就已准备好
...
纯命令语法文档，教 LLM 如何使用 mnemon 命令。与行为引导分离，保持职责清晰。
