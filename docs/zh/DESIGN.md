...
Diff 现已**内置于 `remember`** — 无需单独调用。当调用 `mnemon remember` 时，它会自动在插入前运行 diff 检查。独立的 `mnemon diff` 命令仍然存在，可用于不写入的预检查。

### 8.1 内置 Diff（remember 内部）
...
2. 对每个匹配项计算 ContentSimilarity（token 重叠率或余弦相似度）
3. 检测否定模式：`not`, `no longer`, `don't`, `never`, `switched from`, `replaced`, `不`, `没有`, `放弃`, `改为`
4. 返回建议动作（ADD、UPDATE、CONFLICT 或 DUPLICATE）

### 8.3 典型工作流

有了内置 diff，一条 `remember` 命令即可处理一切：
...
之前的先 `diff` 再 `remember` 的工作流不再是必需的，但仍可通过独立的 `mnemon diff` 命令使用。
